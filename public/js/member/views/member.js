var memberView = Backbone.View.extend({
    ...
    events: {
        "click .btn.btn-primary": "renderRegisterMember",
        "click #searchBtn": "search",
        "click #btnPre": "goPre",
        "click #btnStepOne": "saveBasicInfo"
    },
    initialize: function () {
        this.render();
        this.listenTo(this.collection, "remove", this.renderMember);
        this.listenTo(this.collection, "afterFetcherMembers", this.renderMember);
    },
    render: function () {
        var html = ['<div class="backWhite"><div class="panel panel50">',
            ...
            '</tr></thead>',
            '<tbody id="membersTbody" class="memberscssTb"></tbody></table></div>'];
        this.$el.html(html.join(''));
    },
    renderMember: function () {
        var that = this;
        this.$el.find(".memberscssTb").empty();
        this.$el.find(".pagePosition").remove();
        console.log("this.collection:", this.collection)
        this.collection.each(function (one) {
            console.log("one:", one)
            that.$el.find(".memberscssTb").append(new memberItemView({model:one,collection:this.collection}).el);
        })
        this.$el.find(".backWhite").append(new pageView({collection:this.collection}).el);
    },
    search: function () {
    ...
    renderRegisterMember: function () {
        ...
        var html = ['<div class="col-sm-12">',
            ...
            '<button id="btnStepOne" style="float:right" class="btn btn-default">下一步</button>',//onclick="save(0)"
            '</div>',
